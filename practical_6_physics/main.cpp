...
b2World* world;
float gameHeight = 420.0f;
float gameWidth = 680.0f;
...
void init() {
    ...
    for (int i = 1; i < 11; ++i) {
        ...
        s->setSize(Vector2f(25.0f, 25.0f));
        s->setOrigin(Vector2f(12.5, 12.5f));
    ...
    Vector2f walls[] = {
        ...
        // right
        Vector2f(gameWidth - 25.f, gameHeight * .5f), Vector2f(10.f, gameHeight)
    };

    for (int i = 0; i < 4; i += 1) {
        auto s = new RectangleShape();

            s->setPosition(walls[i*2]);
            s->setSize(walls[(i*2) + 1]);
       
   
        
        s->setOrigin(walls[(i * 2) + 1] / 2.0f);
        s->setFillColor(Color::Green);
        sprites.push_back(s);

        //create static physics body
        auto t = CreatePhysicsBox(*world, false, *s);
        //create boxes
        bodies.push_back(t);
    }
...
int main(){
  sf::RenderWindow window(sf::VideoMode(gameWidth + 200, gameHeight + 200), "SFML works!");
